 ...
 *					매개변수, 지역변수
 *					=> 변수의 초기화가 안된다 (반드시 초기화한 다음에 사용)
 *			------------------------------------------ =>메모리에서 자동으로 저장
 *			Heap : 프로그래머 영역(직접 메모리 크기를 만들어서 저장)
 *					자동으로 소멸이 안된다 => GC(System.gc())
 *			------------------------------------------
 ...
 *						}
 *			-------------------------------------------------
 *			지역변수 		class ClassName   메소드 호출시마다 메소드 안에서만 사용가능
 *						{
 *							void display()
 *							{
 *								int a=10;
 ...
 *			1) 메모리 저장
 *				메모리에 저장된 상태 : 인스턴스(객체)
 *				설계(어떤 데이터를 저장) ==> 메모리에 저장
 *				class ClassName		ClassName c = new ClassName();
 *													  -------- 생성자
 *				{
 *					String s;
 *					int a;
