 ...
 * 			= 선언만 되어있다 => 상속을 내린다 => 상속받은 클래스를 구현해서 사용(오버라이딩)
 * 			= 다중 상속을 지원한다
 * 			상속과정
 * 			interface A
 ...
 * 
 * 			class E extends JFrame implements MouseListener, KeyListenner,ActionListenner, Runnable
 * 			=> Game : 마우스 이동, 키보드 이용, 버튼 이용, 쓰레드
 * 			
 * 		일반메소드
 * 		interface {
 * 			JDK1.8이상 =>
 ...
 * 		}
 * 
 * 		1) 인터페이스는 객체를 생성할 수 없다(구현이 안된 메소드를 가지고 있기때문에 메모리에 저장이 불가능)
 * 			인터페이스 = new 구현한 클래스()
