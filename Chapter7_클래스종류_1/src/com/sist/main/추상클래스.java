 ...
 * 							=> 추상클래스는 단독으로 사용이 불가능
 * 								미완성된 클래스
 * 							=> 사용법은 상속을 내린후에 상속을 받을 클래스를 통해서 메모리 할당이 되는 상태
 * 								class A
 * 								class B extends A
 * 								=> A a = new B();
 * 								=> A a = new A(); => 오류
 ...
 * 	
 * 			5) 선언된 메소드가 있는경우 반드시 구현해야 한다
 * 				
 */
import java.io.*;

abstract class 도형 {
	public abstract void draw(); //무조건 구현해햐한다
	public void color() {
		System.out.println("검정색"); //필요시에 오버라이딩
