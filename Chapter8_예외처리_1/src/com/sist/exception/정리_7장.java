 ...
 * 	   객체지향의 3대 요소
 * 		1. 데이터 보호 목적
 * 			캡슐화 : 변수를 은닉화 => 메소드를 통해서 접근이 가능하게 만듬
 * 				   --------- private 	    ---------- public
 * 				=> 변수는 2가지 기능만 수행
 * 					메모리 저장 / 메모리에서 데이터 읽기
 * 					setter		getter
 ...
 * 			접근 지정어
 * 			1)private 	: 자신의 클래스에서만 사용이 가능, 다른 클래스에서 접근이 불가능(은닉화)
 * 			2)default 	: 같은 패키지 안에서만 접근이 가능
 * 			3)public	: 패키지 상관없이 모든 패키지에서 접근이 가능
 * 		2. 재사용하는 목적 : 상속, 포함
 * 			class A
 ...
 * 	정적 영역
 * 		Method Area / static
 * 	stack  => 메모리 자체 관리, 종료되면 자동 소멸
 * 		지역변수 / 매개변수
 * 	Heap 영역 => 프로그래머 영역(GC) - 사용이 없는 경우, null인 경우 자동 소멸
 * 		객체, 배열, 인터페이스, 열거형
 * 	전체 관리 : JVM
 * 	----------------------------------------------------------------
 */
