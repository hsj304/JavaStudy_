 ...
 * 			읽기 통로 : 데이터가 저자왼 위치로부터 프로그램에서 데이터를 읽어들이기 위한 연결
 ...
 *   File 사용방법
 *   	=생성방법
 *   	 File file = new File("c:\\java\\back.png");
 *   		파일 정보 읽기
 ...
 *   		**mkdir() : 디렉토리 만들기
 *   	 =파일 삭제
 *   		delete() => 한개만 (파일이 있는 디렉토리는 삭제가 안된다 => 파일을 먼저 삭제 => 디렉토리 삭제)
 *   	 =파일이나 디렉토리의 존재여부
 *   		boolean exists()
 *   	=수행날짜
 *   		long lastModified()
...
//		System.out.println("수정 날짜:" + SimpleDateFormat("yy년 MM월 dd일 hh시 mm분 ss초").format());
